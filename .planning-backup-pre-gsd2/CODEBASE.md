...
**Current Phase:** Phase 9 (Architecture Stabilization) — v1.10 shipped
...
| **Class** | `BotState`, `RebootRequestError` |
| **State** | `_is_paused`, `_force_relist`, `_pending_commands`, stats (`cycle_count`, `total_*`, `last_*`) |
...
main.py::run()
  ...
  └─ Main Scan Loop (while running):
      ...
      │   └─ pre-nav at :09:00 before each golden
      │
...
```
TelegramHandler → bot_state.wait_interruptible() returns True
RelistEngine / Main loop → raises RebootRequestError
Main loop → catch RebootRequestError → break loop
...
*Codebase map: 2026-04-20 — v1.9.1 shipped, ~20 source files, ~2600+ lines of application code.*