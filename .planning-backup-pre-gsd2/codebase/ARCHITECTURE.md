...
3. **Golden Sync** → If in golden period (15:10-18:15) and not in relist window, hold until :10. The bot implements a strict **Pre-Nav Guard** at :08:00, waiting until exactly :09:00 before navigating. If already in golden window (:09-:11), skip sleep and relist immediately.
...
- Heartbeat robustness: Uses `Escape` key + `force=True` click on `.icon-transfer` to bypass invisible EA overlays and prevent 3000ms timeouts (in `session_keeper.py`).
