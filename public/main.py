...
def _golden_hold_loop(
    ...
    while seconds_until_pre_nav > 0:
        wait_chunk = max(1, min(300, int(seconds_until_pre_nav)))
        ...
        logger.info(
            ...
            f"alle {next_golden.strftime('%H:%M')}. Check sessione tra {format_duration(wait_chunk)}."
        )
        ...
        while True:
            ...
            if next_golden and is_in_golden_period(now):
                ...
                if seconds_until_pre_nav > 0:
                    ...
                    next_golden = _golden_hold_loop(
                        now, next_golden, page, auth, controller, status_console, logger, cycle
                    )
                    if next_golden is None:
            ...
            if scan.expired_count > 0:
                ...
                if in_hold and not force_relist:
                    ...
                    fifa_logger.info(
                        f"[WAIT ALL EXPIRED] Ci sono ancora {scan.active_count} oggetti attivi o in processing. "
                        "Aspetto che tutti siano scaduti..."
                    )
                    min_active = get_min_active_seconds(scan)
                    if min_active is not None:
                        next_wait = max(min_active - 10, 15)
