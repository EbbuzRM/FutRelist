...
### BUG-002: Heartbeat Timeout due to Overlays ✅ FIXED (2026-04-21)

**Symptom:** The heartbeat mechanism clicks "Transfers" which could go in Timeout (3000ms) due to invisible EA overlays.
**Fix:** Added `Escape` key press before clicking and used `force=True` on the `.icon-transfer` CSS selector to bypass overlays in `session_keeper.py`.
