...
from models.relist_result import RelistBatchResult, RelistResult
...
class RelistExecutor:
    ...
    def __init__(
        self, page: Page, config: dict, rate_limiter: RateLimiter | None = None, auth: AuthManager | None = None
    ):
        self.page = page
        self.config = config
        self.auth = auth  # HI-05: injectato via constructor, no istanze locali
        if rate_limiter:
    ...
    def _click_relist_button(self, listing_index: int) -> None:
        # Final Guard: Ban Prevention — usa auth injectato (HI-05)
        if self.auth and self.auth.is_console_session_active(self.page):
        ...
        listing_el.locator(SELECTORS["relist_button"]).click(force=True)  # ME-10: force=True
    ...
    def relist_all(self, count: int = 1) -> RelistBatchResult:
        ...
        try:
            ...
            if not relist_all_btn.is_visible():
                ...
                return RelistBatchResult(
                    relist_error="Bottone Re-list All non visibile: tutti gli item in Processing o DOM non caricato"
                )

            # ME-10: force=True per bypassare overlay
            relist_all_btn.click(force=True)
            ...
            try:
                ...
                confirm_btn.click(force=True)  # ME-10
            ...
            return RelistBatchResult()  # LO-08: semplificato
        except Exception as e:
    ...
    def check_session_valid(self) -> bool:
        ...
        try:
            if self.auth:
                if not self.auth.is_logged_in(self.page, timeout_ms=5000):
