...
class AuthManager:
    ...
    def check_and_handle_disconnect_modal(self, page: Page) -> bool:
        ...
        Clicca 'Ok' se lo trova e aspetta il redirect alla pagina di login.
            ...
            for dialog in dialogs:
                if dialog and dialog.is_visible():
                    ...
                    if any(kw in text for kw in disconnect_keywords):
                        ...
                        # Attendi che il redirect a signin.ea.com o alla pagina di login sia completo
                        # Aspetta fino a 10 secondi per il cambio di URL
                        try:
                            page.wait_for_url("**/signin.ea.com/**", timeout=10000)
                            logger.info(f"Redirect a signin.ea.com completato: {page.url}")
                        except Exception:
                            # Se il wait_for_url fallisce, controlla l'URL attuale
                            current_url = page.url
                            if "signin.ea.com" in current_url:
                                logger.info(f"Già su signin.ea.com: {current_url}")
                            else:
                                logger.warning(f"Redirect non completato, URL attuale: {current_url}")
                                # Aspetta comunque un po' per permettere il caricamento
                ...
                for selector in logged_in_indicators:
                    try:
                        ...
                        if el and el.is_visible():
                            # Modificato in DEBUG per ridurre il rumore dei log durante gli heartbeat
                            logger.debug(f"Sessione rilevata tramite: {selector}")
                            return True
                    except Exception:
                        continue
    ...
    def perform_login(self, page: Page, email: str, password: str) -> bool:
        ...
        try:
            ...
            # 0.5) Se siamo già su signin.ea.com (es. dopo modale di disconnessione),
            # skip il bottone Login della WebApp e procedi direttamente al login EA
            current_url = page.url.lower()
            if "signin.ea.com" in current_url:
                logger.info("Già su signin.ea.com, procedo con email/password...")
                # Aspetta che la pagina di signin sia caricata
                try:
                    page.wait_for_load_state("networkidle", timeout=10000)
                except Exception:
                    logger.debug("networkidle non raggiunto, procedo comunque")
                # Skip alla parte di inserimento credenziali
                return self._complete_ea_login(page, email, password)
            ...
            # Dopo il click su Login e il redirect a signin.ea.com,
            # verifica se siamo pronti per il completamento del login
            current_url = page.url.lower()
            if "signin.ea.com" in current_url:
                logger.info("Su signin.ea.com dopo click Login, procedo con email/password...")
                return self._complete_ea_login(page, email, password)
            ...
            # Se arriviamo qui, c'è un problema - non siamo su signin.ea.com
            # e non siamo loggati. Log dell'URL attuale per debug.
            logger.error(f"Stato inatteso dopo click Login. URL: {page.url}")
            return False
    ...
    def _complete_ea_login(self, page: Page, email: str, password: str) -> bool:
        """Completa il login EA (email → NEXT → password → Sign in → attendi WebApp).
        
        Da chiamare quando siamo già su signin.ea.com con i campi visibili.
        """
        try:
            ...
            if not email_input.count():
                logger.error("Campo email non trovato su signin.ea.com")
                return False
            ...
            if not next_btn.count():
                # Prova selettori alternativi
                next_btn = page.locator('button[type="submit"], button:has-text("Avanti")')
            if not next_btn.count():
            ...
            if not sign_in_btn.count():
                # Prova selettori alternativi
                sign_in_btn = page.locator('button[type="submit"], button:has-text("Accedi")')
            if not sign_in_btn.count():
            ...
            logger.error(f"Errore durante completamento login EA: {e}")
            return False
